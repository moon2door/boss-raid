...
- `API.Json`: osu! 멀티방 채팅 연결용 개인 설정 파일. IRC 비밀번호가 들어가므로 git에는 올리지 않습니다.
- `API.Json.example`: `API.Json` 작성 예시 파일
...
- `beatmapId`는 `!mp map` 명령에 사용할 beatmap 번호입니다. `link`에서 자동 추출도 시도하지만, 실전에서는 직접 입력하는 편이 안전합니다.

## API.Json 채팅 연결

osu! 멀티방 채팅을 Bridge에서 받아 Unity의 인게임 채팅 영역에 표시하려면 프로젝트 루트 또는 빌드 폴더의 `API.Json`을 수정합니다.

```json
{
  "enabled": true,
  "ircServer": "irc.ppy.sh",
  "ircPort": 6667,
  "ircUsername": "osu_계정명",
  "ircPassword": "osu_IRC_비밀번호",
  "mpChannel": "#mp_123456",
  "commandBotEnabled": true,
  "mpTimerSeconds": 90,
  "mpStartDelaySeconds": 5,
  "readyReminderMessage": "모두 준비를 눌러주세요.",
  "debugRawIrc": false
}
```

- `ircUsername`: osu! 계정명입니다. 공백이 있는 닉네임은 그대로 적어도 Bridge가 IRC용 `_` 형태로 변환합니다.
- `ircPassword`: osu! 로그인 비밀번호가 아니라, osu! 웹사이트 계정 설정에서 확인하는 IRC 비밀번호입니다.
- `mpChannel`: 멀티방 번호입니다. 방 링크가 `https://osu.ppy.sh/community/matches/123456`이면 `#mp_123456`으로 입력합니다.
- `enabled`: `true`로 바꿔야 실제 IRC 연결을 시도합니다.
- `commandBotEnabled`: `true`면 룰렛으로 맵이 확정될 때 `!mp map`, `!mp mods`, `!mp timer`를 자동 전송합니다.
- `mpTimerSeconds`: 준비 타이머 시간입니다. 기본값은 90초입니다.
- `mpStartDelaySeconds`: 모두 준비 완료 후 `!mp start`에 넣을 시간입니다. 기본값은 5초입니다.
- `readyReminderMessage`: 90초가 지나도 모두 준비하지 않았을 때 보낼 채팅입니다.
- `debugRawIrc`: `true`로 바꾸면 Bancho IRC 원문 로그를 모두 출력합니다. 기본값 `false`에서는 접속/방채팅/에러만 표시합니다.

## !mp 명령 봇

Bridge가 IRC에 연결된 상태에서 Unity 룰렛으로 맵이 확정되면 다음 순서로 멀티방 명령을 보냅니다.
...
`NM`은 `!mp mods NF`로 처리하고, `HD`, `HR`, `DT`는 각각 `!mp mods NF HD`처럼 전송합니다. 타이머가 끝나기 전에 BanchoBot의 모두 준비 완료 메시지를 받으면 `!mp aborttimer` 후 `!mp start 5`를 보냅니다. 90초가 지나면 `readyReminderMessage`를 보내고, 이후 모두 준비되면 `!mp start 5`를 보냅니다.

Bridge를 켜면 콘솔에 `API.Json path`와 `[IRC]` 로그가 표시됩니다. 관리자 페이지의 `Room Chat` 패널과 Unity의 `INGAME CHAT` 영역에서 채팅 수신 여부를 확인할 수 있습니다.
